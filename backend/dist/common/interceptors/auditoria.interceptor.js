...
let AuditoriaInterceptor = class AuditoriaInterceptor {
    intercept(context, next) {
        ...
        console.log(`[AUDITORIA] Usuario: ${usuario?.email || 'ANONIMO'} | IP: ${request.ip} | Ruta: ${request.method} ${request.url} | Fecha: ${ahora}`);
        return next.handle().pipe((0, operators_1.tap)(() => {
            console.log(`[AUDITORIA] Finalizado: ${usuario?.email || 'ANONIMO'} | ${request.method} ${request.url}`);
        }));
    }
};
